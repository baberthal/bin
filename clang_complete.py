...
def get_clang_args(projectfolder, projectfilepath, target, sdk='macosx'):
    """
...
def main(argv):
    ...
    if not os.path.exists(projectfolder):
        print '%s does not exist.' % projectfolder
        sys.exit(1)
    ...
    if sdk not in ['macosx', 'iphoneos', 'iphonesimulator']:
        print 'SDK is not correct. It should be macosx / iphoneos / iphonesimulator.'
        sys.exit(1)
    ...
    if None == targetname:
        ...
        print 'Target "%s" is not on the target list %s.' % (targetname, targets)
